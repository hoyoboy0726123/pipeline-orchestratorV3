...
if [[ "$(sudo docker images -q $IMAGE 2>/dev/null)" == "" ]]; then
    ...
    sudo docker build -t "$IMAGE" "$PROJECT_DIR/sandbox"
    echo "✓ 映像檔已建立"
else
    echo "✓ 映像檔已存在：$IMAGE"
    echo "  （如果修改了 Dockerfile / requirements.txt，跑 'sudo docker build -t $IMAGE $PROJECT_DIR/sandbox' 重建）"
fi
...
echo ""
echo "==> 冒煙測試："
if sudo docker exec "$CONTAINER" python -c "import pandas, openpyxl, numpy, requests; print('✓ 核心套件載入 OK')"; then
    echo ""
    echo "══════════════════════════════════════════════════════"
    echo "✓ 沙盒就緒！"
    echo "  容器名：$CONTAINER"
